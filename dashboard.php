...
try {
    // 2. FETCH STATS: Use query() to get totals for the WHOLE school
    $totalStudents = $pdo->query("SELECT count(*) FROM students")->fetchColumn();
    
    // Get the name of the last person added to the database
    $stmtLatest = $pdo->query("SELECT name FROM students ORDER BY id DESC LIMIT 1");
    $latestStudent = $stmtLatest->fetchColumn();
...
<body>
    ...
    <div class="container" style="max-width: 1000px; margin: 0 auto; padding: 40px 20px;">
        <h1 style="color: #2c3e50;">Welcome back, <?= htmlspecialchars($_SESSION['username']) ?>!</h1>
        ...
        <div class="grid">
            ...
            <div class="stat-box">
                ...
                <p style="font-size: 1.3rem; color: #3498db;"><?= $latestStudent ? htmlspecialchars($latestStudent) : 'No Records' ?></p>
            </div>
            <div class="stat-box">
                <h3>System Status</h3>

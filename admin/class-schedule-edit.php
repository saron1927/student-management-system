...
$stmt = $pdo->prepare("SELECT * FROM class_schedule WHERE id = ?");
$stmt->execute([$id]);
$schedule = $stmt->fetch();
if (!$schedule) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $upd = $pdo->prepare("
            ...
            SET teacher_id=?, course_id=?, dept_id=?, section=?, date=?, time_start=?, time_end=?
            WHERE id=?
        ");
        $upd->execute([$teacher_id, $course_id, $dept_id, $section, $date, $time_start, $time_end, $id]);
        header('Location: class-schedule.php?success=' . urlencode('Schedule updated successfully.'));
        exit;
    }
...
<main class="main-content">
    <header class="top-nav">
        <div>
            <h1 style="font-size:1.5rem;font-weight:700;">Edit Class Schedule</h1>
            <p style="color:var(--text-muted);">Update schedule entry #<?= str_pad($id, 4, '0', STR_PAD_LEFT) ?></p>
        </div>
    ...
    <div class="card" style="max-width:760px;">
        <div class="card-header">
            <span style="font-weight:600;"><i class="fas fa-calendar-edit" style="margin-right:.5rem;color:var(--accent);"></i>Schedule Details</span>
        </div>
        <form method="POST" style="padding:1.75rem;display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
            ...
            <div class="form-group">
                ...
                <select name="course_id" id="course_id" class="form-control" required>
                    ...
                    <?php foreach ($courses as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= ($schedule['course_id'] == $c['id']) ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <select name="dept_id" id="dept_id" class="form-control" required>
                    ...
                    <?php foreach ($departments as $d): ?>
                        <option value="<?= $d['id'] ?>" <?= ($schedule['dept_id'] == $d['id']) ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <select name="section" id="section" class="form-control" required>
                    <?php foreach (['A','B','C','D','E'] as $sec): ?>
                        <option value="<?= $sec ?>" <?= ($schedule['section'] === $sec) ? 'selected' : '' ?>><?= $sec ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="time" name="time_start" id="time_start" class="form-control"
                       value="<?= htmlspecialchars(substr($schedule['time_start'], 0, 5)) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="time" name="time_end" id="time_end" class="form-control"
                       value="<?= htmlspecialchars(substr($schedule['time_end'], 0, 5)) ?>" required>
            </div>
            ...
            <div style="grid-column:1/3;display:flex;gap:1rem;justify-content:flex-end;margin-top:.5rem;">
                ...
                <button type="submit" id="btn_update_schedule" class="btn btn-primary" style="width:auto;">

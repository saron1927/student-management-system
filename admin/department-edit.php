...
if (!$dept) {
    header("Location: departments.php?error=" . urlencode("Department not found!"));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($code)) {
        ...
        try {
            // Check if department name or code already exists in OTHER records
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM departments WHERE (name = ? OR code = ?) AND id != ?");
            ...
            if ($stmt->fetchColumn() > 0) {
                ...
                $stmt->execute([$name, $code, $id]);
                header("Location: departments.php?success=" . urlencode("Department updated successfully!"));
                exit();
            }
